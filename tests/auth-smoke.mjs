...
const origin = process.env.SMOKE_ORIGIN || 'http://localhost:4312'
...
try {
  ...
  const invalid = await fetch(`${origin}/api/graphql`, { method: 'POST', headers: { 'content-type': 'application/json', 'cookie': cookieHeader(), 'csrf-token': 'invalid-token' }, body: JSON.stringify({ query: 'mutation{logout{success}}' }) })
  assert.equal(invalid.status, 403, 'invalid CSRF token is rejected')
  const otherPage = await fetch(origin)
  const otherCsrf = (await otherPage.text()).match(/<meta name="csrf-token" content="([^"]+)"/)?.[1]
  assert.ok(otherCsrf)
  const mismatched = await fetch(`${origin}/api/graphql`, { method: 'POST', headers: { 'content-type': 'application/json', 'cookie': cookieHeader(), 'csrf-token': otherCsrf }, body: JSON.stringify({ query: 'mutation{logout{success}}' }) })
  assert.equal(mismatched.status, 403, 'another browser session cannot supply the CSRF token')
  ...
  const blockedLogout = await fetch(`${origin}/api/graphql`, { method: 'POST', headers: { 'content-type': 'application/json', 'cookie': cookieHeader() }, body: JSON.stringify({ query: 'mutation{logout{success}}' }) })
  assert.equal(blockedLogout.status, 403, 'authenticated mutations require the CSRF token')
  assert.equal((await graphql('{currentUser{id}}')).data.currentUser.id, userId, 'rejected mutation cannot log out the user')
  ...
  console.log('PASS: CSRF missing/invalid/mismatched tokens rejected, authenticated mutation protected, anonymous query, register payload/cookie, authenticated query/SSR/cache, logout, login payload, authenticated WebSocket query/subscription, browser login/reload/logout')
}
finally {
